'''
    딕셔너리
        사전
        형태 : (key1:value,key2:value,....)
            >key와 value 한 쌍
            >key = 단어, value = 뜻
        -문자열 포매팅 함수(format)에서 {키워드} 사용
            "{name}".format(name="홍길동")
        -요소가 여러개 나열된 자료형
        문자열,리스트,튜플의 경우 순서가 있어서 인덱싱 슬라이싱 가능
...
del(my_dict["16강"])#요소 삭제 시,key를 입력
print(my_dict)
...
my_set={1,2,3,1,2,3,1,2,3,1,2,3}
print(my_set)
my_set2={2,1,2,1,2,1,2,1,"A","A","B","B"}
print(my_set2)
...
set()
'''
my_str=str(123123)#정수를 문자취급하여 문자열로 변환
print(my_str+"1234")
...
test={1,2,3,1,2,3,1,2,3,1,2,3,1,2,3,4,6,7,8,6,5,4,5,3,4,6,9}
