...
#(1) 무한반복
#항상 조건이 만족하여 반복문이 끝나지 않는다. ==> 무한반복현상
#Ctrl + C : 강제종료
...
while num2<=10:
    num1+=num2
    num2+=1
print("1~10까지의 합은 {}입니다.".format(num1))

'''
    2. 1부터 입력 받은 수 까지 합 구하기
    [출력결과]
    숫자 입력 : 5
    1~5까지 합은 15입니다.
'''
...
while num5<num3:
    num3+=num4
    num5+=1
print("1~5까지의 합은 {}입니다.".format(num4,num3))
'''
...
#특정 조건 만족
'''
input_num=[1,2,3] # 초기값이 9면 while 문을 수행못함
#9가 아닌 아무 값이나 넣은것
#보통 프로그래밍 언어에서 변수를 만들고 초기화할 때 0을 사용
while input_num !=9: #9가 아니면 만족
    input_num =int(input("9를 입력하면 종료 :"))
    num6+=1
    num7+=input_num

print("평균은 {:.2f} 입니다.".format(num7/num6))
'''
...
while numb<10:
    print("{}*{}={}".format(dan,numb,(dan*numb)))
    numb+=1
'''
...
print("200이상일 때 soo2 : {}   sum : {}".format(soo2,soo1))



#continue (while 문의 조건식으로 점프)

numbers=1

while numbers<10: #numbers 값이 10보다 작으면 만족 -> 수행
    if numbers%2==0: #numbers를 2로 나눈 나머지가 0과 같냐 (짝수)
        numbers+=1
        continue
    #continue를 한다는 것은 '수행문을 끝내는 것'
    #새로운 종료 지점 생성

    print("numbers={}".format(numbers))#짝수는 위 조건문에 의해 출력 X
    numbers+=1
...
#   if문 없는 continue = 무조건 조건식 이동 = continue 아래 코드 의미 x
...
while ast3<=100:
    if ast3%3!=0 and ast3 % 5 !=0: 
        ast3+=1
        continue
        
    ast3+=1
    asta+=ast3
print("1~100까지의 3의 배수 합은 {}".format(asta))
...
#for 1~100까지 홀수의 합 구하기

sum2=0
for hap2 in range(1,101,2):
    sum2+=hap2
print("1~100까지 홀수의 합 :",sum2)
'''    
...
for dang in range(6):
    ...
    print("{}번 째 공은 : {}".format(dang+1,rnd))
...
for j in range(2,10):#단 : 2~9
    print("\n{}단 ".format(j))
    #j에 2가 대입된 상태로 i의 for문을 수행
    #>>i의 for문이 전체 수행되고 끝이 나야 j가 2일때 수행이 끝이난다.
    for i in range(1,10):
        print("{} * {} = {}".format(j,i,2*i))
'''
print()
for i in range(1,4):
    for j in range(1,4):
        print("i={},j={}-j의 for문 1회 수행 끝".format(i,j))
        for k in range(1,4):
            print(i,j,k)
'''
...
'''
hap3=int(input("숫자 입력 :"))
sum3=0
for k in range(1,hap3):
    if k % 2 ==0:
        sum3+=k
print("1~{}까지 홀수의 합은 {}입니다. :",format(hap3,sum3))
'''

'''
    1부터 200까지 3과 4의 공배수를 하나의 변수에 '누적'
    누적된 수가 1000을 초과하면 반복문을 '탈출'
    이 때, 누적된 수와 마지막에 더했던 공배수를 출력
...
'''
    1~100 사이 정수 중, 3의 배수와 5의 배수를 '역순'으로 출력
...
'''
    2중 for문 구구단 예제를 for문 1개만 사용해서 만들어보기
        - 총 반복 횟수 = 72회
        - 처음 단은 2
        - 곱해지는 숫자는 처음이 1
