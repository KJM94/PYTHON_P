...
[파일 입출력] - File Input/Output
    ...
    -파일의 이해
        ...
        -우리가 코드로 파일을 다룰 때 기본적으로 메모장으로 여는 것처럼
...
#닫지 않는다면 프로그램이 계속 사용중이므로 다른 곳에서 파일을 다룰 수 없다.
file.close()
'''
...
with open("c:\\PT\\test.txt","r") as file : #open한 파일을 file 변수로 다룸
    text=file.read()
    print(text)
#close를 생략해도 with문법 안에서 자동으로 close해준다(open을 사용한 경우)
'''

#파일 내용을 한 줄씩 읽기(1)
#readlines() : 한 줄씩 문자열로 나눠서 리스트 반환(개행의 \n 포함)
'''
...
#한 번읽거나 ,쓰고나면 자동으로 다음 위치로 offset 이동
...
#파일에서 10글자 이하 단어 갯수 세기
'''
with open("C:\\PT\\dan.txt","r") as file:
    cnt=0
    words=file.readline()
    for dan in words:
        if len(dan.strip('\n')) <= 10:
...
'''
    C://test_practice.txt 파일을 미리 만들어놓고 아래 정보 구하기
    [출력결과]
    전체 글자 수 : ??
    전체 단어 수 : ??
    전체 라인 수 : ??
...
#write도 마찬가지로, 자동으로 쓰고 난 다음 위치로 offset 이동
